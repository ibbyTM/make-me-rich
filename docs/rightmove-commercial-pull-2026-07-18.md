...
**Date:** 2026-07-18 · **Source:** rightmove.co.uk commercial-property-for-sale (search pages, `__NEXT_DATA__` plain JSON) · **Mode:** read-only live pull

Searched **7 cities** (Citywide footprint: Yorkshire + Greater Manchester). Portal reports **870** matching results across those searches; fetched a capped **556** (max 5 pages/city, 1.5 s between requests), **556** after cross-city dedupe. **94** business-for-sale going concerns excluded by subtype, leaving **462** investable listings. Stage-0 (minimum bar **2**, geography excluded from scoring — see below) passed **187**.
...
**155 distinct agents/branches** appear in the 556 fetched listings (counted before the subtype filter — coverage is a property of the portal, not of our filtering) — vs one agent per bespoke source. 91 distinct agents appear in the Stage-0-passed set.
...
275 investable listings scored below the bar (geography not scored — these counts reflect price/size/keyword signals only).

| Best score | Listings | Typical shortfall |
|---|---|---|
| 0/2 | 96 | no non-geo signal at all (POA price, no size given, no keyword hit) |
| 1/2 | 179 | one signal only — e.g. keyword but price outside budget / size unknown |
